import { StoryItem, ThumbnailLoadingState } from "@/type/story-types";
...
export const useStories = (story: StoryItem[]) => {
  ...
  const [thumbnailsLoading, setThumbnailsLoading] =
    useState<ThumbnailLoadingState>({});
    ...
    if (modalVisible && story.length > 0) {
      Object.keys(videoPlayersRef.current).forEach((keyString) => {
        ...
        if (currentPlayer && key !== selectedIndex) {
          safeVideoOperation(
            () => currentPlayer.pause(),
            "Pause non-selected video failed",
          );

          if (currentPlayer.timeline?.seek) {
            safeVideoOperation(
              () => currentPlayer.timeline.seek(0),
              "Seek non-selected video failed",
            );
          }
        }
      });
